...
class CommentsController extends GetxController {
  ...
  try {
    // 1️⃣ Get the comment document first
    final commentSnapshot = await commentRef.doc(commentId).get();

    if (!commentSnapshot.exists) return;

    // 2️⃣ Check if it has a parentCommentId (means it's a reply)
    final parentId = commentSnapshot.data()?['parentCommentId'];

    int deleteCount = 1; // Always delete at least this comment

    // 3️⃣ If it's a top-level comment (no parent), delete its replies too
    if (parentId == null || parentId.isEmpty) {
      final repliesSnapshot = await commentRef
    ...
    // 4️⃣ Delete the original comment
    await commentRef.doc(commentId).delete();

    // 5️⃣ Decrement the total comment count
